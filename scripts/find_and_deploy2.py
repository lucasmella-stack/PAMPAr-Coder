...
for gpu_id, cloud in GPU_ATTEMPTS:
    query = {
        "query": f'mutation {{ podFindAndDeployOnDemand(input: {{ name: "pampar-train", gpuTypeId: "{gpu_id}", gpuCount: 1, cloudType: {cloud}, containerDiskInGb: 40, volumeInGb: 50, imageName: "{IMAGE}", ports: "22/tcp,8888/http" }}) {{ id desiredStatus machine {{ podHostId gpuDisplayName }} }} }}'
    }
