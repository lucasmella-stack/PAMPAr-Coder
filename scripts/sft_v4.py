...
"""
sft_v4.py — Second-pass SFT de PamparV3 con datos dirigidos.

Diferencias respecto a sft_v3.py:
  - Parte de v3_sft.pt (no del pretraining)
  - Usa SOLO el dataset dirigido (targeted_sft.jsonl) con pares
    exactos problema→funcion donde el nombre coincide
  - LR mucho mas bajo: 5e-6 -> 5e-7
  - Loss masking: solo computa loss sobre la parte "### Solution:" en adelante
  - Max 3000 pasos (datos pequenos, alta calidad)
  - Guarda en v3_sft_v4.pt

Uso:
  python -X utf8 scripts/sft_v4.py
"""
...
logger = logging.getLogger("sft_v4")
...
def _guardar(ruta: Path, modelo: PamparV3, optimizer: torch.optim.Optimizer, paso: int) -> None:
    ...
    logger.info("Checkpoint SFT-v4 guardado -> paso %d", paso)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--targeted", type=Path,
                   default=ROOT / "data" / "targeted_sft.jsonl")
    p.add_argument("--lr", type=float, default=5e-6)
    p.add_argument("--lr-min", type=float, default=5e-7)
    p.add_argument("--warmup", type=int, default=50)
    p.add_argument("--max-pasos", type=int, default=3000)
    p.add_argument("--epochs", type=int, default=20,
                   help="Muchas epochs sobre los pocos datos de calidad")
...
def main() -> None:
    ...
    logger.info("Cargado desde %s (tipo: %s)", args.checkpoint_in.name,
                payload.get("tipo", "?"))
    del payload
    ...
    if not args.targeted.exists():
        ...
        logger.error("Primero ejecuta: python scripts/generate_targeted_sft.py")
        sys.exit(1)
    ...
    print(f"\n  Evaluar con:")
    print(f"    cd PAMPAr-Coder && python -X utf8 scripts/eval_v3.py --checkpoint checkpoints/v3_sft_v4.pt")
